...
const mockNewLaunchData: NewLaunchToken[] = [
  {
    token: {
      ...
      image: "https://www.believescreener.com/_next/image?url=ht…qtrdxa4soa6xzcadsgdxpqrqz4go3i4yoanzsim&w=96&q=75"
    },
    curvePercentage: "0",
    contract: {
      address: "Dgkc...iBLV",
      tradeLink: "https://axiom.trade/t/DgkcLphzsXZcD4qVRFGrfQbvaKgRcdeKNwU7T8htiBLV/@bscreener"
    },
    twitter: {
      handle: "@BSubkhi32009",
...
const TokenRow: React.FC<{ token: NewLaunchToken; theme: string }> = ({ token, theme }) => {
    ...
    <TouchableOpacity className="flex-row items-center py-4 px-5">
      <View className="flex-row items-center flex-1 mr-2">
        <View className="w-10 h-10 rounded-full mr-3 overflow-hidden">
          <Image 
            ...
            defaultSource={{ uri: 'https://via.placeholder.com/40x40/6B7280/FFFFFF?text=' + token.token.symbol.charAt(0) }}
          />
        </View>
      ...
      <View className="items-center justify-center w-16 mx-1">
        <View className={`w-8 h-8 rounded-full justify-center items-center`}
          ...
          <Text className={`text-xs font-bold`} style={{ color: getCurveColor(curveValue) }}>
      ...
      <View className="items-center justify-center w-20 mx-1">
        <TouchableOpacity 
          className={`px-3 py-1.5 rounded-full border ${
            theme === 'dark' ? 'border-blue-500 bg-blue-500/10' : 'border-blue-600 bg-blue-50'
          }`}
        >
          <Text className={`text-xs font-medium ${theme === 'dark' ? 'text-blue-400' : 'text-blue-600'}`}>
            TRADE
          </Text>
        </TouchableOpacity>
      </View>
      ...
      <View className="items-center justify-center w-16 mx-1">
        ...
        >
          <Text className={`text-xs ${theme === 'dark' ? 'text-blue-400' : 'text-blue-600'}`}>
      ...
      <View className="items-center justify-center w-10">
        <View className={`px-1.5 py-1 rounded-md ${
...
export default function NewLaunchScreen() {
  ...
  const NewLaunchData = filteredData ? filteredData.data.otherData.newLaunchTableData : mockNewLaunchData || [];

  const filteredTokens = NewLaunchData.filter(token =>
